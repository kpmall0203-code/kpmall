...
function onOpen() {
  ...
  ui.createMenu('아마존 → KSE')
    ...
    .addSubMenu(ui.createMenu('설정')
      ...
      .addItem('개별 검사 — 금액만 (단가·합계 어긋남)', '검사_금액만')
      .addSeparator()
...
function 배송상태_자동조회() {
  ...
  // 배송등록(자동 이어보내기)과 겹치면 완료 이관이 행을 지워 접수번호가 엉뚱한 행에 적힌다 — 겹치면 이번은 쉰다
...
function 도움말() {
  var html = HtmlService.createHtmlOutput(
    ...
    '너무 자주 걸리는 낱말은 판정을 <b>애매</b>로 바꾸면 확정(빨강)이 아니라 노랑으로 칠해 ' +
    '사람이 판단할 건으로 [오류확인] 에 모입니다. 메뉴는 <b>설정 > 주소 낱말 목록 열기</b>.</p>' +
    '<p><b>③ 주문 정렬</b><br>상품 하나인 박스는 상품명 기준으로 같은 물건끼리 모으고, ' +
    '같은 상품 안에서는 <b>발송기한이 급한 것을 위로</b> 올립니다. 합배송 박스는 맨 아래에 ' +
    '수취인별로 모읍니다.</p>' +
    '<p style="color:#666">④ 배송등록은 이 시트의 <b>아래 행부터</b> 접수합니다. KSE는 접수한 ' +
    '차례대로 번호를 매기고 목록·송장을 뽑으면 최근 접수가 앞에 오므로, 급한 건을 마지막에 ' +
    '접수해야 뽑았을 때 앞 장에 옵니다. 시트의 보기 순서(급한 것이 위)는 그대로입니다. ' +
    'KSE 목록이 오래된 접수부터 나오면 [설정] <b>KSE_등록순서</b>를 \'시트 위부터\'로 바꾸세요.</p>' +
    '<p style="color:#666">④는 한 실행(6분) 안에 보낼 수 있는 만큼만 보내고, 남은 건은 <b>1분 뒤 ' +
    '자동으로 이어서</b> 보냅니다 — 대기 줄이 없어질 때까지. 확인 창을 오래 띄워 두었으면 그 ' +
    '실행에서는 보내지 않고 바로 자동 실행에 넘깁니다 (보내다 끊기면 KSE에만 접수되고 시트에 ' +
    '안 남기 때문). 진행은 [로그] 시트에 남습니다.</p>' +
