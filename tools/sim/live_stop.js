...
// 손잡이(광고ID)는 아직 없다 — ① 을 돌려야 생긴다. 판정 셈만 보려고 전부 있다고 친다
global.adUnitMap_=()=>{const m={};
  for(const r of (cells['광고실적']||[]).slice(1)){const s=String(r[1]||'').trim();
    if(s&&!m[s]) m[s]={ads:[{id:'AD_'+s,gid:'G',cid:'C',camp:String(r[3]||''),grp:'',state:'ENABLED'}],on:1};}
  return m;};
...
if(!outRows){ alerts=[]; buildAdStopCandidates(); }
