...
function adEnableStep_(interactive) {
  ...
  var col = [];
  for (var c0 = 0; c0 < v.length; c0++) col.push([v[c0][AP_RESULT - 1]]);
  var dirty = false, since = 0;
  var flushCol = function () {
    if (!dirty) return;
    sh.getRange(2, AP_RESULT, col.length, 1).setValues(col);
    dirty = false;
  };
  var logBuf = adLogBuffer_(ADEXEC_FLUSH_EVERY);
  ...
  for (var i = 0; i < v.length; i++) {
    ...
    if (gid) {
      try {
        ...
        for (var a = 0; a < arr.length; a++) {
          if (String(arr[a].state) !== to && arr[a].adId) ids.push(String(arr[a].adId));
        }
    ...
    for (var b = 0; b < ids.length; b += ADEXEC_ADS_BATCH) {
      ...
      try {
        ...
        if (rr.ok) nAds += rr.ids.length; else bad.push('상품');
