...
const bySrc={}; const ex=[];
for(const r of rows){ const k=r[col('마진출처')]; bySrc[k]=(bySrc[k]||0)+1;
  if(k==='마진율 시트') ex.push(r[0]+' '+r[col('마진율(%)')]+'%'); }
console.log('출처', JSON.stringify(bySrc));
console.log('상품 목록(이름 매칭)에서 온 것:', ex.slice(0,8).join(' | '));
