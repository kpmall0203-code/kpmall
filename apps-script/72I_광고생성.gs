 ...
 * 판정에 필요한 클릭 N = 3 ÷ 필요CVR, 필요CVR = CPC ÷ 손익분기CPA 이므로
 *   하루 예산 = N × CPC ÷ 14 = 손익분기CPA × 3 ÷ 14
 * CPC 가 약분되어 사라진다 — 비싼 클릭이면 적게, 싼 클릭이면 많이 사면 되고
 * 판정에 드는 총액은 '손익분기 주문 3건어치'로 같기 때문이다. 여기에 여유 배수를
 * 곱하고, 오가닉 매출 대비 목표 ACOS 를 넘지 않도록 한 번 더 깎는다.
 *
