...
var ADBASIS_DEFAULTS = [
  ['기본 마진율', 0.15, '원가도 [광고 기준값] 시트도 없는 SKU 에 쓴다. 실제로 남는 비율(수수료·배송비 뺀 뒤). ' +
                       '2026-09-09 에 0.17 에서 0.15 로 내렸다 — 이미 적어 둔 값이 있는 상품은 그 값을 쓰므로 안 바뀐다'],
  ...
  ['시장가 안전배수', 1.2, 'CPC 상한이 시장 CPC × 이 값 이상이어야 캠페인에 넣는다. ' +
                        '시장가에 사도 세션 마진이 얼마쯤 남아야 하기 때문 — 1.2 면 20% 여유. ' +
                        '못 넘는 SKU 는 광고로 이길 수 없으니 오가닉으로 둔다'],
  ...
  ['주간 광고비 한도(JPY)', '', '트랙 A(재배분) 캠페인이 7일에 쓸 수 있는 돈. 비우면 승인 ✓ 일예산 합 × 7. ' +
                              '넘으면 관제가 트랙 A 만 멈추고 메일. ' +
                              '⚠ 트랙 B(육성)는 여기에 넣지 않습니다 — SKU 마다 육성 표의 [주간허용손해] 로 따로 봅니다'],
  ['한도 경고 비율', 0.8, '한도의 이 비율에 닿으면 미리 메일 (아직 멈추지는 않음)'],
  ['한도 넘으면 자동 멈춤', 'TRUE', 'FALSE 로 하면 넘어도 메일만 보내고 멈추지 않는다 — 권하지 않음'],
  ['판단전환율 사전클릭', 50,
   '표본이 적을 때 초기 추정 쪽으로 끌어당기는 무게. 클릭 1회 주문 1건을 전환율 100% 로 ' +
   '받아 과입찰하는 것을 막는다. 판단전환율 = (성숙주문 + 이 값 × 초기추정) ÷ (성숙클릭 + 이 값)'],
  ['트랙 B 자동 겨냥', '검색어만',
   '트랙 B 자동 캠페인에서 살릴 겨냥. 자동은 넷으로 돈다 — ' +
   '유사검색어 · 넓은검색어 · 대체상품 · 보완상품. ' +
   '뒤의 둘은 남의 상품 페이지에 붙는 광고라 검색어 순위와 아무 상관이 없다 — ' +
   '트랙 B 가 사려는 것에 기여하지 않으므로 끈다. ' +
   '값: "검색어만"(기본, 앞의 둘) · "유사검색어만"(가장 좁게) · "전부"(넷 다)'],
  ['(안 씀) 1페이지 순위', 16, '지금은 아무 데도 안 씁니다. 트랙 B 는 순위로 판정하지 않습니다. 옛 안에 있는 상품을 받지 않는다 — ' +
                      '순위가 이미 있으면 살 것이 없고, 손익분기를 넘겨 사면 잠식만 산다'],
  ['캠페인 이름 앞머리', 'KP', '만든 캠페인을 한눈에 알아보려고 붙인다. ' +
   '아마존은 캠페인 이름에 아스키(영문·숫자)만 받으므로 한글을 쓰면 무시되고 KP 가 된다. ' +
   '⚠ 이 값을 바꾸면 이미 만든 캠페인을 못 찾아 같은 것을 또 만든다 — 처음 한 번만 정하세요'],
  // ── 확대(EXPAND) — 기존 광고를 더 사 볼 때의 규칙 (기획서 8장) ──
  // 통계·기간 값은 제안값이고, 돈에 관한 넷은 비워 둔다. 비면 시험이 시작되지 않는다.
  ['확대 · 모드', '모의운영', '모의운영 = 계획만 세운다 · 자동운영 = 승인한 시험을 실제로 보낸다'],
  ['확대 · 1회 인상폭', 0.10, '한 번에 올릴 폭. 0.10 이면 지금 값의 110% 까지 (기획서 4.3)'],
  ['확대 · 이익보존계수', 0.65, '목표 입찰 상한 = 주문당 공헌이익 × 판단주문율 × 이 값. ' +
   '손익분기까지 다 쓰지 않고 일부를 이익으로 남긴다'],
  ['확대 · 기준기간(일)', 14, '바꾸기 전 자료를 몇 일 볼까 (같은 요일 수를 맞춘다)'],
  ['확대 · 운영기간(일)', 14, '바꾼 값으로 며칠 돌릴까. 끝나면 원래 값으로 되돌린다'],
  ['확대 · 대조군 비율', 0.20, '적격 상품군의 이만큼은 일부러 안 바꾸고 견주는 데 쓴다. ' +
   '이것이 없으면 "올려서 늘었다" 와 "그냥 그 주가 좋았다" 를 구별할 수 없다'],
  ['확대 · 대조군 seed', 20260908, '누구를 대조군으로 할지 정하는 씨앗. 바꾸면 배정이 통째로 바뀐다 — 고정하세요'],
  ['확대 · 최소 상품군 수', 30, '양쪽 군이 각각 이만큼은 돼야 확증 판정을 낸다 (기획서 5.3)'],
  ['확대 · 부트스트랩 횟수', 1000, '추가이익 구간을 재표집으로 낼 때의 횟수'],
  ['확대 · 구간 수준', 0.90, '0.90 이면 90% 구간'],
  ['확대 · 최소 유의미 이익(JPY/상품군/일)', '',
   '이보다 커야 "늘었다" 고 본다. 비우면 0 을 기준선으로 보되 판정은 미확정으로 둔다'],
  ['확대 · 최대 동시 시험 수', '', '비우면 제한 없음. 한 번에 몇 개까지 돌릴까'],
  ['확대 · 시험 주간 지출한도(JPY)', '',
   '비우면 제한 없음. 확대가 7일에 더 쓸 수 있는 돈 — 증액 시험의 [예약액] 과 승격의 ' +
   '주간 추가액이 같은 이 주머니에서 나갑니다. 넘는 것은 다음 차례로 미룹니다'],
  ['확대 · 시험 손실한도(JPY)', '',
   '비우면 제한 없음. 지금 돌고 있는 시험들이 시작한 뒤 낸 손해(광고비 − 광고매출 × 마진율)의 ' +
   '합계 상한입니다 — 주간도 월간도 아니고, 그 시험들이 도는 동안(최대 ' +
   '[확대 · 운영기간(일)])의 누계입니다. 넘으면 손해가 큰 시험부터 즉시 되돌리고, ' +
   '벌고 있는 시험은 그대로 둡니다'],
  ['확대 · 최대 유효입찰(JPY)', '',
   '⚠ 비우면 시험을 시작하지 않습니다 (이것만 필수). 어떤 계산이 나와도 이 값을 넘겨 부르지 ' +
   '않습니다 — 마진율이나 판매가를 잘못 적으면 목표 상한은 얼마든 커질 수 있고, ' +
   '그것을 막는 마지막 잣대가 이 하나입니다'],
  ['확대 · 승격 첫 배수', 1.5,
   '승격(가격선 캠페인으로 꺼내기) 할 때 처음 부를 값의 상한 배수. 1.5 면 지금 내던 값의 150% 까지만 ' +
   '올려 시작하고, 목표 가격선까지는 그 뒤 평범한 증액 시험(한 번에 ' +
   '[확대 · 1회 인상폭] 만큼)이 한 계단씩 올린다. 구조를 바꾼 것과 값을 크게 올린 것을 ' +
   '한꺼번에 하면 무엇 덕분인지 알 수 없다'],
  ['확대 · 승격 반반 시험', 'TRUE',
   'TRUE 면 가격선마다 캠페인을 둘 만들어 절반은 값 그대로(대조편), 절반은 첫 배수(시험편 …T)로 ' +
   '시작하고 30일 뒤 첫 배수가 정말 버는지 판정합니다. 같은 ASIN 은 같은 편입니다. ' +
   'FALSE 면 전부 첫 배수로 시작합니다 (재지 않습니다)'],
  ['확대 · 실패 냉각기간(일)', 28, '효과가 없던 상품을 며칠 뒤에 다시 볼까'],
  ['확대 · 상품별로 판단해 이어가기', 'FALSE',
   'TRUE 면 상품군이 30개에 못 미쳐 "미확정" 이라도, 그 상품의 대조군 보정 변화가 [최소 유의미 이익] 을 ' +
   '넘으면 채택하고 다음 계단을 올라간다. 통계로 확증된 것은 아니다 — 규모가 작을 때 실용적으로 쓰는 값'],
  ['마진율 시트 ID', '1ZOgOkmI53oebHB_8rkiYNV4Fk67el6P9SYGOtq2M6-4',
   '바깥 참고 시트. 두 탭을 씁니다 — [광고 기준값] 은 SKU/ASIN 으로 맞대는 원장(마진율·판매가)이고, ' +
   '아래 [마진율 시트 탭] 은 일본어 상품명으로만 맞대는 옛 자료입니다. 비우면 둘 다 안 읽습니다'],
  ['마진율 시트 탭', '상품 목록', '이름으로 맞대는 탭 (SKU 가 없어 정확히 같은 이름일 때만 붙는다)'],
