 ...
 *
 * ── 빈 한도는 무제한이 아니다 ───────────────────────────
 * 한도 칸이 비어 있으면 '아직 안 정함' 이다. 그 대상은 새로 켜지지도, 증액되지도
 * 않는다. 기존 캠페인 예산의 합이나 시트에 남아 있던 옛 숫자를 승인된 한도로
 * 삼지 않는다. 실제로 광고육성 표에는 주간허용손해 4,500엔이 적혀 있지만
 * 그것은 과거 설정이지 이번 정책의 승인값이 아니다.
 *
 * ── 머리글로 열을 찾는다 ────────────────────────────────
...
/**
 * 트랙마다 반드시 채워져야 하는 한도.
 * 트랙 B 는 지출과 손실이 다른 값이다 — 주문이 0이면 손실은 광고비 전액이므로
 * 지출한도만으로는 손실을 막지 못한다.
 */
var POLICY_NEED = {
  A: ['주간 지출한도(JPY)'],
  B: ['주간 지출한도(JPY)', '주간 손실한도(JPY)',
      '누적 지출한도(JPY)', '누적 손실한도(JPY)', '최대 기간(일)']
};
...
function setupAdPolicy() {
  ...
  ui_().alert('운영 정책',
    ...
    '한도 칸이 비어 있으면 "아직 안 정함" 입니다 — 그 대상은 새로 켜지지도 증액되지도 않습니다.\n' +
    '무제한이나 기존 예산 합으로 대신 읽지 않습니다.\n\n' +
