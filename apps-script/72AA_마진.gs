...
function adMarginProgram_(ctx, sku, price, jpName) {
  ...
  /**
   * 판매가가 소싱 줄과 같으면 그 줄의 마진을 그대로 쓴다.
   *
   * 그 칸은 **수수료·배송비·조달비가 이미 다 빠진 값**이다 (사용자 확인 2026-09-11).
   * 그러면 수수료율도 배송비도 우리가 가정할 일이 없다 — 가정을 하나도 안 쓰는 길이
   * 있으면 그 길로 간다. 실제로 419줄 가운데 416줄(99.3%)이 리스팅 판매가와 같다.
   *
   * 값이 달라졌을 때만 아래에서 배송비를 거꾸로 꺼내 수수료만 다시 센다 (64B 와 같은 셈).
   * 그 길은 시트가 쓴 수수료율이 우리 10%% 와 다르면 그 차이를 배송비로 흡수한다 —
   * 값이 같을 때는 결과가 똑같고, 달라진 만큼만 (판매가 차이 × 수수료율 차이) 어긋난다.
   */
  if (sc && sc.margin > 0 && sc.price > 0 && price > 0 && Math.abs(price - sc.price) < 1) {
    var pct0 = sc.margin / price * 100;
    if (pct0 > 0 && pct0 < 100) {
      return { pct: Math.round(pct0 * 10) / 10, src: MSRC_SRC,
               why: '소싱 시트에 적힌 마진 ¥' + Math.round(sc.margin) + ' ÷ 판매가 ¥' +
                    Math.round(price) + ' 입니다 — 수수료·배송비·조달비가 이미 빠진 값이라 ' +
                    '우리가 다시 셈하지 않습니다 (조달비 ' + Math.round(sc.krw).toLocaleString() + '원)' };
    }
  }
 ...
 *
 * ── 이 함수를 언제 쓰나 ─────────────────────────────────
 * 판매가가 소싱 줄과 같으면 아예 안 쓴다 — 그때는 그 줄의 마진을 그대로 쓴다.
 * 리프라이싱으로 값이 달라진 줄에서만 부른다 (2026-09-11 현재 419줄 중 3줄).
 *
