...
var NA_CFG_DEFAULTS = [
  ...
  ['신규 · 상품군 탐색배수', 1.0,
   '판돈 = min(위 상한, 주문당공헌이익 × 이 값). 1.0 이면 "주문 한 건 이익만큼 써 본다" · 0.5 면 그 절반'],
  ['신규 · 시드 주문율', 0.02,
  ...
  ['신규 · 이익보존계수', 1.0,
   '허용 입찰 = 주문당공헌이익 × 판단주문율 × 이 값. 1.0 = 클릭당 기대이익만큼(손익분기)까지 · 0.5 = 그 절반. ' +
   '손실은 판돈이 막는다 — 이 값은 "얼마나 빨리 근거를 사나" 다'],
  ['신규 · 시작 비율', 1.0, '시작 입찰 = 허용 입찰 × 이 값. 1.0 이면 허용값에서 바로 시작 (탐색 조정이 올릴 여지는 없다)'],
  ['신규 · 최대 유효입찰(JPY)', '',
...
function naPolicy_() {
  ...
  var p = {
    ...
    weekSpend: num('신규 · 탐색 주간 지출한도(JPY)', 300000),
    famPot: num('신규 · 상품군 탐색비 상한(JPY)', 1000),
    famMult: num('신규 · 상품군 탐색배수', 1.0),
    q0: num('신규 · 시드 주문율', 0.02),
    beta: num('신규 · 이익보존계수', 1.0),
    startFrac: num('신규 · 시작 비율', 1.0),
