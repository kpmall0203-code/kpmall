...
function adTermVerdict_(o, basis) {
  ...
  if (o.orders === 0 && N > 0 && o.clicks >= N) {
    return { v: '부정', why: '클릭 ' + o.clicks + '회 — 판정에 필요한 ' + N + '회를 넘겼는데 주문 0' + src,
             cpc: cpc, needCvr: needCvr, N: N };
  }
  /**
   * 클릭 수가 아니라 쓴 돈으로 보는 갈래.
   *
   * 처음에는 '손익분기 주문 두 건어치'로 뒀는데, 판정필요클릭 × CPC = 손익분기CPA × 3 이므로
   * 두 건어치는 언제나 판정필요클릭의 3분의 2 지점이다. 그러면 이 규칙이 항상 먼저 걸려
   * 클릭 규칙이 죽고, 판정 지점이 통계로 정한 자리보다 앞당겨진다.
  ...
  if (o.orders === 0) {
    return { v: '더 봄', why: '클릭 ' + o.clicks + '회 — 판정에 필요한 ' + (N || '?') + '회에 못 미칩니다' + src,
             cpc: cpc, needCvr: needCvr, N: N };
  }
...
function fetchAdSearchTerms() {
  ...
  var wins = adAskWeeks_('검색어 수집',
    ...
    '주를 합쳐 검색어마다 한 줄로 판정합니다 — 클릭이 주를 넘어 쌓여야 판정에 필요한 수에 닿습니다.\n' +
 ...
 *
 * 왜 합치나: 판정필요클릭은 보통 200 안팎이다. 한 주에 그만큼 클릭하는 검색어는 드물어
