...
function tabModes_() {
  return [
     ...
     * "무엇을 보고 있는지" 를 알 수 없다.
     *
     *   새 상품 키우기  사람이 고른 몇 개에 일부러 손해를 보며 순위를 사는 자리
     *   기존 광고       근거 있는 상품의 수익을 키우는 자리 — 멈춤·감액·증액 사다리·승격
     *   자료            프로그램이 읽고 쓰는 수집물. 사람은 무엇이 언제 들어왔는지만 본다
     *   (신규 상품의 표는 별도 파일 'Amazon 신규 상품 광고' 에 있다 — 이 통합문서엔 계획 줄만 온다)
     *
     * 어느 탭이 무엇인지는 [탭 안내] 가 한 줄씩 적어 준다.
     */
    { key: 'adsB', name: '🌱 광고 · 새 상품 키우기 (골라서 순위 사기)',
      tabs: [SHEET_DASH, SHEET_ADCHART, ADSTAT_PROG_SHEETS.N, ADSTAT_PROG_SHEETS.A, ADSTAT_PROG_SHEETS.B, SHEET_ADGROW, SHEET_POLICY, SHEET_JOB, SHEET_INBOX,
...
function tabGuideRows_() {
  ...
  return [
    ...
    [C, ADSTAT_PROG_SHEETS.N, '🆕 신규만 따로 — 요약 · 풀(캠페인)별 최근 30일 표 · 그래프 3개 (광고비·광고매출·TACOS / ACOS·TACOS / 클릭·주문)',
     TAB_WHO_PROG, '보고 싶을 때'],
    [C, ADSTAT_PROG_SHEETS.A, '🔁 기존만 따로 — 요약 · 캠페인별 최근 30일 표 · 그래프 3개',
     TAB_WHO_PROG, '보고 싶을 때'],
    [C, ADSTAT_PROG_SHEETS.B, '🌱 키우기만 따로 — 요약 · 캠페인별 최근 30일 표 · 그래프 3개',
     TAB_WHO_PROG, '보고 싶을 때'],
    ...
    [A, SHEET_POLICY, '옛 트랙 A(재배분)의 한도·모드·승인. 운영 현황의 "기존 광고 주간 지출한도" 가 이것을 읽는다. ' +
     '새 상품 키우기의 한도는 광고육성 표 안에, 기존 광고(확대)와 신규의 한도는 광고기준·신규 설정에 있다',
     TAB_WHO_HUMAN, '거의 안 봄'],
    ...
    [B, SHEET_ADPLAN_GROW, '만들 새 상품 키우기 캠페인 줄. 승인 ✓ 인 줄만 만들어지고 켜진다',
     TAB_WHO_BOTH, '캠페인이 안 만들어질 때'],
    [B, SHEET_SPENDDAY, '캠페인 × 날짜 지출·매출 원장. 여력·손실 계산은 이 표 하나만 본다',
     TAB_WHO_PROG, '자료가 며칠 것인지 확인할 때'],
    [C, SHEET_ADWATCH, '매일 08시 아마존에 물어 적는 캠페인별 상태·승인·7일 지출. 한도 초과·승인 없는 켜짐은 멈추고 메일. ' +
     '요약은 운영 현황에 실린다 — 이 표는 [멈춤] 을 체크해 여러 개를 끌 때 연다',
     TAB_WHO_BOTH, '운영 현황이 이상하다 할 때'],
    [B, SHEET_ADTERM, '자동 캠페인이 실제로 산 검색어와 판정. 기준키워드 자동 선정이 여기서 판 말을 고른다',
     TAB_WHO_BOTH, '주 1회'],

    [C, SHEET_ADBASIS, '광고 프로그램 셋이 함께 읽는 기준값 — 기본 마진율 · 확대(기존 광고)의 한도·모드 · 신규광고 시트 ID 등',
     TAB_WHO_HUMAN, '기준을 바꿀 때'],
    [A, SHEET_EXPAND, '기존 광고의 앞 표 — 사람이 보는 표는 이것 하나다. 상품마다 마진율(사람이 고치면 지켜진다)과 ' +
     '[판정](멈춤·증액 시험·감액·대조군·분리 필요·천장·유지)과 [바꿀 것] 이 있고, [승인] 을 켜면 [② 시작] 이 그대로 보낸다',
     TAB_WHO_BOTH, '주 1회 — 새 줄의 승인을 켤 때'],
    ...
    [A, SHEET_ADSTOP, '멈춤 셈의 자세한 표 (본전 주문율 · 기대 · 상한). 판정은 앞 표(광고확대후보)에 같이 적힌다 — ' +
     '이 표는 왜 그렇게 판정했는지 볼 때만 연다',
     TAB_WHO_PROG, '멈춤 판정의 근거를 볼 때'],
    [A, SHEET_REALLOC, 'SKU 별 채산성(손익분기 CPA · CPC 상한 · 권장 CPC). 옛 트랙 A 의 첫 표였고 지금은 검색어 판정이 기준으로 읽는다 — 주 1회 자동 계산',
     TAB_WHO_PROG, '거의 안 봄'],
    [A, SHEET_ADPLAN, '만들 캠페인 줄 — 기존 광고 승격(트랙 X) · 신규 상품(N) · 검색어 승격(M). 승인 ✓ 인 줄만 만들어진다',
     TAB_WHO_BOTH, '캠페인이 안 만들어질 때'],
    [A, SHEET_ADPROD, '어느 광고그룹이 어느 SKU 를 광고하나 — 채산성과 입찰을 잇는 다리',
     TAB_WHO_PROG, '이상할 때만'],
    [A, SHEET_SALES, '세션·전환율의 출처. SKU 채산성 계산이 이것을 본다', TAB_WHO_PROG, '이상할 때만'],
...
function showTabGuide() {
  ...
  ui_().alert('탭 안내',
    ...
    '   🔁 기존 광고 (멈춤·감액·증액)\n' +
    '   📥 광고 자료 (수집물)\n' +
    '   (신규 상품의 표는 별도 파일 "Amazon 신규 상품 광고" 에 있습니다)',
    ui_().ButtonSet.OK);
}
